...
self_score: 8
predecessor_score: 9
active_task: D5 is fixed and on main (0a3b488 fix, tests and gate; f9fb312 the missing-credentials note the operator chose; 06023d8 and aadb299 moved citations, README and gate; 0ba0bea plan). D2 is the last defect in plan section 4, recommended next. Next: D2, or any other open item (untracked files, CLAUDE.md purpose fence, seven unpinned findings now including an API-side one, methodology sync, coverage floor, live API test blocked on credentials, CARTO tile key, backlog BL-1, BL-2, BL-3, or the learning 15 gate).
what_was_done: Claimed on fix/d5-one-credential off main da040da (98d9530). Probed first: probe/d5.py starts a fresh app per mix of .env variables with every proxy variable on a closed port, reads four routes over HTTP, and headless Chrome reads the page. 4 apps (unfixed; A, the four variables read in active_source(); B, active_source() calls get_api_client(); C, B plus a note naming the missing credentials) x 7 mixes = 28 runs. Unfixed, with AIRQINO_CLIENT_ID set and a credential missing: "API Connected", no banner, grid stuck on "Loading readings...", every route 503; with a CSV too, "API Connected" over CSV readings. A and B matched on every mix once memory addresses were masked; C matched B apart from the note. The operator picked C. Test-first: marker removed, 1 failed; widened to five half-filled cases and a CSV case, 5 failed and 1 passed on the unfixed code. 0a3b488: active_source() calls get_api_client() (app.py:67 now); T1.5 sets all four credentials (it pinned D5); tests-passed 129 -> 135. f9fb312: API_CREDENTIALS and missing_api_credentials() (app.py:54-60), the amber note (templates/dashboard.html:40-46) and its CSS, 9 page tests (7 red, 2 green before). 06023d8 and aadb299 moved the citations the added lines displaced (app.py +10, template +8, README +2), added a README sentence and set tests-passed 135 -> 144. 0ba0bea amended the plan. Red-drives against the whole suite, files restored byte-identical: fix with the original marker and T1.5 (only those two fail), unfixed with the new tests (5 fail), design A (passes), a two-of-four check (2 fail), a note shown with no credentials (1 fails), a note only in the banner (2 fail); the gate with a test hidden measured 134 and exited 2. main fast-forwarded, branch deleted, pushed after the close-out commit. Suite: 144 passed, 2 xfailed, exit 0. quality_ratchet: 2/2 pass · 0 fail · 0 unmeasured · results 2993256f954d · manifest 0f1d28b61292
next_steps: Fix D2 next (the operator may pick another open item): int() on ?hours= in /api/timeseries (app.py:188) and ?days= in /api/hourly (app.py:237); xfails tests/test_csv_routes.py:192 (marker :191) and tests/test_api_routes.py:261 (marker :260). The timeseries parse runs before any source check, so it 500s with no source; the hourly one needs an API client. The page's range buttons send numbers only (templates/dashboard.html:94-99), so a bad value comes from a typed URL. The operator picks the design (a 400 naming the parameter, or the default); probe the candidates in the real app first (learning 12), and check the chart's range buttons still work after the fix. Remove both markers, watch red, fix, tighten tests-passed from 144 by the measured delta, red-drive with the markers restored against the whole suite, and after each commit that adds lines grep every surface for citations into every file it touched.
key_files: app.py:67 (active_source's API check, D5's fix), app.py:54 (API_CREDENTIALS and missing_api_credentials, to :60), app.py:188 (D2 timeseries), app.py:237 (D2 hourly), templates/dashboard.html:40 (the note, to :46), tests/test_api_routes.py:222 (D5's comment and tests, to :253), tests/test_api_routes.py:260 (D2's hourly marker), tests/test_csv_routes.py:191 (D2's timeseries marker), tests/test_dashboard_page.py:122 (the note's tests, to :160), .quality-gates.json:27 (tests-passed, 144), docs/planning/test-suite-plan.md:683 (As implemented D5 note), CLAUDE.md:60 (learning 18)
gotchas: Session 20's probe (d5.py, a fresh app per mix of variables with the proxy guard and an optional CSV upload, and drive.mjs, which reads the badge, banner, grid, note and chart) is in its scratchpad's probe/ directory (path in SESSION_NOTES.md); start headless Chrome on port 9333 yourself and stop it with pkill -f remote-debugging-port=9333. Added lines move citations in other files: the note's template lines and the README's moved citations in a test comment and in BACKLOG.md, found one commit late; grep every surface for every touched file. Mask memory addresses (0x...) before comparing probe output. active_source() now calls get_api_client(), so a test that installs fake_client reads as api with no credentials set. The suite takes about 4 s while headless Chrome runs and 0.57 s otherwise. No git hooks are installed, so run quality_ratchet.py --precommit by hand with the manifest staged. The shell is zsh: redirect output to a file, then echo $? (learning 10). SESSION_NOTES.md must stay at 399 lines or fewer by wc -l.
runtime_smoke: Verified live. The repo's app, a fresh app for each of the 7 mixes (none; CLIENT_ID alone; 3 of 4 with and without CLIENT_ID; all four; CLIENT_ID plus a CSV; all four plus a CSV), headless Chrome on each page: badge, banner, grid, note and the four routes matched the probed design C on every mix, and the only vendor 502s (12) came from the 2 all-four runs, where the proxy guard refused the token request. Chrome and every app stopped; ports 5001 and 9333 free. Not covered: real credentials (none yet).
changelog_ref: CHANGELOG.md "2026-09-18 · [ad hoc] Session 20 closed out — D5 fixed with a missing-credentials note, added to the plan; main fast-forwarded, pushed with this commit"
commit: 0a3b488
```
...
self_score: 7
predecessor_score: 9
active_task: D8 (the serial-reader race Session 18 found, added to plan section 4 by the operator) is fixed and on main (b53303e fix, test and gate; 927dfe8 the tests' moved app.py citations; 1af2c49 plan). D5 and D2 remain, D5 recommended next. Next: D5, or any other open item (untracked files, CLAUDE.md purpose fence, six unpinned findings now including the get_api_client() race, methodology sync, coverage floor, live API test blocked on credentials, CARTO tile key, backlog BL-1, BL-2, BL-3, or the learning 15 gate).
what_was_done: Claimed on fix/serial-reader-race off main 34a0a75 (052cc5a). Probed first, reusing Session 18's scripts: a pty feeding the docstring's line, a fresh app per trial with no warm-up, the page's two first requests sent together, 5 trials per candidate. Unfixed: 2 fds on the pty every time, 2 of 14 readings clean; a lock: 1 fd, 23 of 23 clean; exclusive=True: 1 fd but 503 "Could not exclusively lock port" in 4 of 5. The driven page after a reload: unfixed lost its CO card, the lock showed all five, exclusive=True told the user to restart. An API probe: get_api_client() built 3 clients from 3 concurrent calls (constructor slowed), and one shared client still sent 3 token requests, so a lock there prevents nothing. The operator named it D8 and kept get_api_client() out of scope. Test-first: a slow stand-in reader, two threads behind a Barrier; unfixed 20 of 20 red (assert 2 == 1). b53303e: a module-level threading.Lock in get_serial_reader() (app.py:22, :40-51); tests-passed 128 -> 129; 20 of 20 green. Red-drives against the whole suite: unfixed and a check-only lock fail just the new test, double-checked locking passes, the test without its sleep passed on the unfixed code 10 of 10 (the delay is the plant; its comment says so); the gate with the test hidden measured 128 and exited 2. 927dfe8 moved eight test citations. 1af2c49 amended the plan (D8 row, note) and corrected b53303e's ledger count (2 of 14, not 2 of 15). Runtime on the repo app: 5 fresh apps, 1 fd, 22 of 22 clean; the page shows all five cards; bad-port and no-source paths unchanged. main fast-forwarded, branch deleted, pushed after the close-out commit. Suite: 129 passed, 3 xfailed, exit 0. quality_ratchet: 2/2 pass · 0 fail · 0 unmeasured · results 9706974b83af · manifest 501e3c8538de
next_steps: Fix D5 next (the operator may pick D2 or another open item instead): remove the marker at tests/test_api_routes.py:224 and watch the test at :227 fail; active_source() reports api on AIRQINO_CLIENT_ID alone (app.py:58) while get_api_client() needs all four credentials (app.py:34). A real D5 fix also fails Phase 1's T1.5 (tests/test_dashboard_page.py:97), which sets only AIRQINO_CLIENT_ID, so that session sets all four credentials in T1.5 too. Probe each candidate on the real page first (learning 12): the badge, the setup banner and /api/current with one credential, then all four. D2 after it: app.py:178 and :227; xfails tests/test_csv_routes.py:192 (marker :191) and tests/test_api_routes.py:233 (marker :232). Recount every number from the saved output with a script before the commit that states it (learning 17).
key_files: app.py:22 (_serial_lock), app.py:40 (get_serial_reader, the with at :42, to :51), app.py:58 (the AIRQINO_CLIENT_ID check D5 cites), tests/test_serial_routes.py:71 (D8's comment and test, to :106), tests/test_api_routes.py:224 (D5's marker), tests/test_dashboard_page.py:97 (T1.5), .quality-gates.json:27 (tests-passed, 129), docs/planning/test-suite-plan.md:142 (D8's row), docs/planning/test-suite-plan.md:659 (As implemented D8 note), CLAUDE.md:59 (learning 17)
gotchas: Session 19's probe scripts (race.py, page.py, drive.mjs, a non-blocking pty_feed.py) are in its scratchpad's probe/ directory (path in SESSION_NOTES.md) and take the app directory as their first argument. A planted delay can be the whole test: red-drive the test with its plant removed on the unfixed code (D8's passed 10 of 10 without its sleep). A garbled serial line can still parse: count clean rows against the exact line fed. The page's first load shows "Loading readings..." until a reload or the 60 s refresh, so a page probe must reload. No git hooks are installed, so run quality_ratchet.py --precommit by hand with the manifest staged. The shell is zsh: redirect output to a file, then echo $? (learning 10). SESSION_NOTES.md must stay at 399 lines or fewer by wc -l.
runtime_smoke: Verified live. The fixed repo app with a pty on SERIAL_PORT: 5 fresh apps with the page's two first requests sent together held the pty once each, and all 22 readings were clean (unfixed: twice each, 2 of 14 clean). The headless-Chrome DevTools driver on a fresh app, then a reload: all five reading cards including CO and two 5-point series, 1 fd. A bad SERIAL_PORT still shows D7's error in the grid; no source still gives 503s and the setup banner. App, Chrome and the pty feeder stopped; ports 5001 and 9333 free. Not covered: a real adapter.
changelog_ref: CHANGELOG.md "2026-09-18 · [ad hoc] Session 19 closed out — D8, the serial-reader race, fixed and added to the plan; main fast-forwarded, pushed with this commit"
commit: b53303e
```
...
self_score: 7
predecessor_score: 9
active_task: D6 (plan section 6's fifth fix session) is fixed and on main (5056055 fix, test and gate; 91ea569 plan; 2943371 a D5 citation the fix moved). D5 and D2 remain. The D6 probe also found a serial-reader race outside plan section 4 (two concurrent first requests start two SerialReaders on one port), recorded but not fixed. Next: the race (recommended; then D5, D2), or any other open item (untracked files, CLAUDE.md purpose fence, five unpinned findings, methodology sync, coverage floor, live API test blocked on credentials, CARTO tile key, backlog BL-1, BL-2, BL-3, or the learning 15 gate).
what_was_done: Claimed on fix/d6-source-order off main be44a02 (2160d6b). Probed first on the real page: a pty feeding sensor lines on SERIAL_PORT plus four fake credentials, every proxy variable pointed at a closed port so a vendor request would fail as a 502 (none did). Unfixed, the badge said "API Connected" over five serial reading cards and /api/status said api; the reorder, run in a scratchpad copy, changed only the badge, to "Serial". The first readings were garbled: lsof showed the app holding the pty twice, and a scripted A/B (fresh app per trial) confirmed a race in get_serial_reader() (2 fds after 3 of 3 concurrent first requests, 1 after 3 of 3 sequential); recorded, not fixed. Only one design fits the xfail and README, so no picker. Test-first: D6's xfail became a test over the four mixes of two or more sources; unfixed, serial+api and all-three failed. 5056055: active_source() checks SERIAL_PORT before AIRQINO_CLIENT_ID (app.py:53-56); tests-passed 124 -> 128. 3 product red-drives and 1 gate red-drive against the whole suite. Runtime on the fixed repo app: the driven page, then a fresh app for each of 7 source mixes, badge, /api/status and both data routes agree. 91ea569 updated the plan and recorded the race. 2943371 corrected a false claim in that note (the swap moved the AIRQINO_CLIENT_ID check from :53 to :55, which D5's xfail cites). main fast-forwarded, branch deleted, pushed after the close-out commit. Suite: 128 passed, 3 xfailed, exit 0. quality_ratchet: 2/2 pass · 0 fail · 0 unmeasured · results 0002758d231d · manifest f7714b97d99b
next_steps: Fix the serial-reader race next (the operator may pick D5 or D2 instead): get_serial_reader() (app.py:38-48) checks _serial_reader, then builds and starts a reader with no lock, and the page's first load sends /api/current and /api/timeseries together (static/js/dashboard.js:484-485). The operator decides whether it becomes D8 in the plan. Write the test first and plant the race (learning 9): a stand-in SerialReader whose constructor sleeps, two threads released by a threading.Barrier, assert one reader built. Candidate fix, not verified: a module-level threading.Lock around the check and the build (exclusive=True on the port would surface a port error instead). Probe from a fresh app with no warm-up request, and count the pty's fds with lsof before and after (learning 16). get_api_client() (app.py:24-35) has the same shape, unprobed. D5: app.py:55, xfail tests/test_api_routes.py:227 (marker :224); a real D5 fix also fails T1.5 (tests/test_dashboard_page.py:97), which must then set all four credentials. D2: app.py:175 and :224; xfails tests/test_csv_routes.py:192 (marker :191) and tests/test_api_routes.py:233 (marker :232).
key_files: app.py:53 (active_source, the D6 swap to :56), app.py:38 (get_serial_reader, the race, to :48), app.py:55 (the AIRQINO_CLIENT_ID check D5 cites), tests/test_api_routes.py:152 (D6's comment, BADGES and the four-mix test, to :183), tests/test_api_routes.py:224 (D5's marker), tests/test_api_routes.py:232 (D2's hourly marker), static/js/dashboard.js:484 (the two concurrent first loads, to :485), .quality-gates.json:27 (tests-passed, 128), CLAUDE.md:58 (learning 16), docs/planning/test-suite-plan.md:634 (As implemented D6 note and the race)
gotchas: Warming the reader with one request before loading the page hides the race; a race probe must start a fresh app and load the page first (learning 16). A swap keeps line numbers but moves content: after a change, grep every surface for citations into the lines changed (D6's swap moved the check D5 cites from :53 to :55). Flask's debug reloader runs two processes: start a probe's app with start_new_session=True and stop it with os.killpg, or kill both PIDs from lsof -tiTCP:5001. No git hooks are installed, so run quality_ratchet.py --precommit by hand with the manifest staged. For probes with fake credentials, point HTTPS_PROXY, HTTP_PROXY and their lowercase forms at http://127.0.0.1:9 for the app only, so a vendor request shows as a 502. The shell is zsh: redirect output to a file, then echo $? (learning 10). SESSION_NOTES.md must stay at 399 lines or fewer by wc -l.
runtime_smoke: Verified live. The fixed app from the repo with a pty on SERIAL_PORT and four fake credentials (proxy guard on): the headless-Chrome DevTools driver read "Serial" over five serial reading cards and two chart series (before the fix: "API Connected" over the same cards). A fresh app per source mix (none, serial, api, csv, serial+api, api+csv, serial+csv): the badge, /api/status, /api/current and /api/timeseries agree in each, and none shows the banner except none. No 502 in any app log. App, Chrome and the pty feeder stopped; ports 5001 and 9333 free. Not covered: a real adapter and real API credentials.
changelog_ref: CHANGELOG.md "2026-09-18 · [ad hoc] Session 18 closed out — D6 fixed, the plan's fifth fix session done; a serial-reader race found and recorded; main fast-forwarded, pushed with this commit"
commit: 5056055
```
...
self_score: 7
predecessor_score: 9
active_task: D4 (plan section 6's fourth fix session) is fixed and on main (caec75a fix, tests, README and gate; 9812ef3 plan; 18e94c3 escape correction). Next: D6 (recommended; then D5, D2), or any other open item (untracked files, CLAUDE.md purpose fence, five unpinned findings, methodology sync, coverage floor, live API test blocked on credentials, CARTO tile key, backlog BL-1, BL-2, BL-3, or the new item 10: make learning 15 a gate).
what_was_done: Claimed on fix/d4-csv-bom off main 9995f62 (d63e4f9). Probed first with learning 14's drag-and-drop driver: unfixed, a BOM CSV read "Loaded 5 rows" in green but the cards had no timestamps and the chart had no series; with pm25 as the first column, the PM2.5 card and series were missing (a second symptom plan section 4 doesn't list). The utf-8-sig candidate, run in a scratchpad copy of the app, fixed both. No design picker: the one alternative (strip U+FEFF from each header key) gives the same page. Test-first: both D4 tests red on the unfixed code. caec75a: app.py:249 decodes as utf-8-sig (same line); D4's marker removed, its test pins body and row, a new test for a semicolon file with pm25 first, a README clause, tests-passed 122 -> 124. 4 product red-drives and 1 gate red-drive against the whole suite. 9812ef3 updated the plan. At close-out a byte grep found that the Edit tool had decoded the escape typed for the BOM into the character, so caec75a's tests still held literal BOMs while its ledger entry claimed the escape; 18e94c3 rewrote them with Python and a new ledger entry records the false claim. Runtime before and after on the real page, plus adjacent paths. main fast-forwarded, branch deleted, pushed after the close-out commit. Suite: 124 passed, 4 xfailed, exit 0. quality_ratchet: 2/2 pass · 0 fail · 0 unmeasured · results 5de9dfd7b3bc · manifest c0a107040a76
next_steps: Fix D6 next: active_source() checks the API before serial (app.py:53-58), but /api/current and /api/timeseries try serial first (app.py:141, :179), and README.md:36 documents serial -> API -> CSV. Its xfail is tests/test_api_routes.py:205 (marker :202). Before choosing the design, run the real app with a pty feeding a sensor line on SERIAL_PORT plus four fake credentials, and look at the header badge beside the readings grid, with the current code and the candidate (learning 12). By reading, not probed: with AIRQINO_PROJECT_NAME and AIRQINO_STATION_NAME unset, the page's /api/metadata call makes no client call (app.py:117, :129), so the probe should send nothing to the vendor; check that first. Then the recipe: remove the marker and watch red, fix, tighten tests-passed by the measured delta, red-drive with the marker restored against the whole suite and run the suite on the fix alone (learning 11), re-grep citations. D5 also changes active_source(); D6's test sets all four credentials, so a D5 fix can't flip it.
key_files: app.py:249 (the D4 decode), app.py:53 (active_source, D5 and D6), tests/test_csv_routes.py:175 (D4's comment and two tests, to :186), tests/test_csv_routes.py:191 (D2's marker), tests/test_api_routes.py:202 (D6's marker), .quality-gates.json:27 (tests-passed, 124), README.md:40 (CSV paragraph), CLAUDE.md:57 (learning 15), docs/planning/test-suite-plan.md:624 (As implemented D4 note)
gotchas: The Edit tool decodes a backslash-u escape in its input into the character: the BOM escape typed for D4's tests landed as a literal BOM in Python and Markdown alike, while backslash-n stayed as two characters. Write such text with a script that builds it (chr(92) + "ufeff"), then grep the bytes (learning 15). Flask's debug reloader runs two processes, so lsof -tiTCP:5001 prints two PIDs; a one-PID command needs a loop, and check which app copy answers by its cwd. Upload state persists in a running app. Redirect exit codes to a file, then echo $? (learning 10). Stage files by name (4 untracked files remain). SESSION_NOTES.md must stay at 399 lines or fewer by wc -l (386 now).
runtime_smoke: Verified live. A real python3 app.py with no .env, scratchpad CSVs (CRLF, as Excel writes) dropped on the page by a headless-Chrome DevTools script, before and after. Before: a BOM file with timestamp first read "Loaded 5 rows (5 columns)" in green, the cards had no timestamps and the chart had no series, also after a reload; with pm25 first, no PM2.5 card or series. After, from the repo: timestamps on all four cards and two 5-point series for both files and after a reload. Adjacent: a clean CSV unchanged; curl of a BOM file with a ragged row returns clean columns and ragged_rows 1; an invalid-UTF-8 file still loads with U+FFFD. App and Chrome stopped; ports 5001 and 9333 free (lsof exit 1). The escape correction changed no behaviour: the suite passes and both D4 tests fail on the unfixed decode.
changelog_ref: CHANGELOG.md "2026-09-18 · [ad hoc] Session 17 closed out — D4 fixed, the plan's fourth fix session done; main fast-forwarded, pushed with this commit"
commit: caec75a
```
...
self_score: 8
predecessor_score: 9
active_task: D3 (plan section 6's third fix session) is fixed and on main (31e50f2 page, 5e37cb1 server, tests, README and gate, c3004b7 plan). Next: D4 (recommended; then D6, D5, D2), or any other open item (untracked files, CLAUDE.md purpose fence, five unpinned findings, methodology sync, coverage floor, live API test blocked on credentials, CARTO tile key, or backlog BL-1, BL-2, BL-3).
what_was_done: Claimed on fix/d3-ragged-csv-row off main a1cb7ec (5c93025). Probed first, with a scratchpad Node script that drives headless Chrome over the DevTools protocol and drops a CSV on the page (--screenshot can't upload): the real app gave a 500 as Werkzeug's HTML page, so the page read "Upload failed: Unexpected token '<'..." and loaded nothing. Three designs probed on the page: A drop the extras silently, B drop and count them, C reject with a 400; the operator chose B. Two commits: 31e50f2 (the upload status adds the ragged-row count in amber when the response has ragged_rows) and 5e37cb1 (upload_csv skips the None key and counts those rows; ragged_rows only when not 0; D3's marker removed, its test pins the new body, one new test; a README sentence; tests-passed 120 -> 122). c3004b7 updated the plan. 6 product red-drives and 1 gate red-drive, all against the whole suite. Runtime before and after, plus a reload and a clean upload. main fast-forwarded, branch deleted, pushed after the close-out commit. Suite: 122 passed, 5 xfailed, exit 0. quality_ratchet: 2/2 pass · 0 fail · 0 unmeasured · results 4c59862276a3 · manifest 6947b3732825
next_steps: Fix D4 next: app.py:249 decodes as utf-8, not utf-8-sig; xfail at tests/test_csv_routes.py:185 (marker :183). A BOM stays in the first header, and the page reads row.timestamp for the chart (static/js/dashboard.js:270) and data.timestamp for the cards (:202). Before choosing the design, drop a BOM-prefixed CSV on the real page with the current code and each candidate, using learning 14's driver, and look at the status, cards and chart (learning 12). Then the recipe: remove the marker and watch red, fix, tighten tests-passed by the measured delta, red-drive with the marker restored against the whole suite and run the suite on the fix alone (learning 11), re-grep citations and measure line shifts.
key_files: app.py:260 (upload_csv parse loop; the fix at :262, :266-268, :277-283), app.py:249 (D4's decode), static/js/dashboard.js:449 (uploadFile; the ragged note at :461-467), tests/test_csv_routes.py:158 (D3's two tests), tests/test_csv_routes.py:183 (D4's marker), .quality-gates.json:27 (tests-passed, 122), README.md:40 (CSV paragraph), CLAUDE.md:56 (learning 14, the driver recipe), docs/planning/test-suite-plan.md:602 (As implemented D3 note)
gotchas: An upload needs a driver, not a screenshot (learning 14). In the real app (debug=True) a 500 is HTML, so the page's resp.json() throws and the status shows a JSON parse error. Upload state persists in a running app, and the upload area moves from the setup banner to the bottom panel once a source is active: restart the app between probes that need the no-source page. zsh expands an unquoted --include=*.py in grep -r ("no matches found"): quote it. A line shift after an edit must be measured, not computed. SESSION_NOTES.md must stay at 399 lines or fewer by wc -l (374 now). Redirect exit codes to a file, then echo $? (learning 10). Stage files by name (4 untracked files remain).
runtime_smoke: Verified live. A real python3 app.py with no .env, a 5-row CSV with 2 ragged rows dropped on the page by a headless-Chrome DevTools script, before and after. Before: "Upload failed: Unexpected token '<', "<!doctype "... is not valid JSON", nothing loaded. After: "Loaded 5 rows (5 columns). 2 rows have more fields than the header; the extra fields were ignored." in amber, four reading cards and two 5-point chart series. Adjacent: a reload shows the CSV Data badge and the data; a clean CSV on the bottom upload area reads "Loaded 5 rows (5 columns)" in green, and curl returns no ragged_rows. App and Chrome stopped; port 5001 free.
changelog_ref: CHANGELOG.md "2026-09-18 · [ad hoc] Session 16 closed out — D3 fixed, the plan's third fix session done; main fast-forwarded, pushed with this commit"
commit: 5e37cb1
```
