...
### 2026-09-18 · [ad hoc] Session 20 closed out — D5 fixed with a missing-credentials note, added to the plan; main fast-forwarded, pushed with this commit
- **Change:** `SESSION_NOTES.md` carries the Session 19 handoff evaluation (9/10; nothing wrong found; missing the CSV symptom and how to start headless Chrome for `drive.mjs`), the full D5 write-up and an 8/10 self-assessment (template and README citation drift caught one commit late; a plan-note overclaim caught before its commit). Open item 1 now recommends D2, the last defect in plan §4, with current citations (`app.py:188`, `:237`; markers `tests/test_csv_routes.py:191`, `tests/test_api_routes.py:260`). Open item 4 gains a seventh finding: with all four credentials set and the token request failing, the page says "API Connected" over "Loading readings...". Every open item's citation into a file this session changed was moved and printed against the file. `CLAUDE.md` gains learning #18: a summary of a choice the code makes elsewhere should call the function that makes it, not re-test its inputs. The S20 `HANDOFFS.md` receipt is `status: complete`. FM #28 reduction: "Session 17 Handoff Evaluation" and "What Session 18 Did" archived (`git show 0ba0bea:SESSION_NOTES.md`). On the operator's direction (the landing picker), `main` was fast-forwarded from `da040da` to `0ba0bea` with `git merge --ff-only`, the fully merged local branch `fix/d5-one-credential` (tip `0ba0bea`, never pushed) was deleted with `git branch -d`, and `main` is pushed to `origin/main` straight after this commit
- **Commit/PR:** the close-out commit (ships this entry); session commits `98d9530`, `0a3b488`, `f9fb312`, `06023d8`, `aadb299`, `0ba0bea`
- **Session:** S20 · **Verified:** after `git fetch`, `origin/main` = `da040da` was an ancestor of the branch before the merge, and afterwards `git merge-base --is-ancestor` confirmed each of the six session commits is in `main`; `python3 -m pytest -q` gives `144 passed, 2 xfailed`, exit 0; `quality_ratchet: 2/2 pass · 0 fail · 0 unmeasured · results 2993256f954d · manifest 0f1d28b61292`; dashboard 68/100, the same 6 flags as at Phase 0, listed from `dashboard.html` after the close-out edits; every `file:line` in the new handoff text printed against the current files; `wc -l SESSION_NOTES.md` is 391; the receipt's `changelog_ref` matches this heading
...
- **Commit/PR:** this commit
- **Session:** S20 · **Verified:** each new citation printed against `app.py` lands on the line it names, and each unmoved one still does; `python3 -m pytest -q -rx` gives `144 passed, 2 xfailed`, exit 0, and the two xfail reasons print the new lines
- **Model:** Claude Opus 5 (claude-opus-5[1m])

### 2026-09-18 · [ad hoc] The page names the missing API credentials when `.env` sets some but not all four
- **Change:** the operator's pick in the Session 20 design picker (C: D5's fix plus a note). `app.py:54-60`: `API_CREDENTIALS` and `missing_api_credentials()`, which returns the unset credentials when 1 to 3 of the four are set, else `[]`; `dashboard()` passes it to the template (`:85`). `templates/dashboard.html:40-46`: an amber note at the top of the page, "The cloud API needs all four credentials in `.env`. Missing: …", shown whatever the source, so it also appears beside CSV or serial data and when only `AIRQINO_CLIENT_ID` is missing, which before the fix gave "No Data Source" with no hint. `static/css/dashboard.css:75-80`: its style, in the existing `--moderate` amber. `tests/test_dashboard_page.py:122-160`: `test_note_names_the_missing_credentials` (7 cases: none, all four, `CLIENT_ID` only, each 3 of 4) checks the note's exact text or its absence, and `test_note_shows_beside_another_source` (CSV, serial). The 10 added lines move every `app.py` line from the old `:54` down (by 9, and by 10 from the old `:76`), so five citations in three test files now read low; the next commit moves them. `tests-passed` stays 135 here and tightens with the README commit
- **Commit/PR:** this commit
- **Session:** S20 · **Verified:** the note tests before the note existed: 7 failed, 2 passed (none and all four). With the note: `144 passed, 2 xfailed`, exit 0, from a file with no pipe. Red-drives against the whole suite in the scratch worktree, files restored byte-identical (`shasum -c`): a note that ignores "some", so it also shows with no credentials set, 1 failed (`[none]`); a note only inside the setup banner (`missing_credentials and not source`), 2 failed (`[csv]`, `[serial]`). Runtime: the repo's app, a fresh app per mix for the probe's 7 mixes, headless Chrome on each page: the page and all four routes matched the probed design C on every mix (memory addresses masked); the 12 vendor 502s came from the 2 all-four runs, where the proxy guard refused the token request
- **Model:** Claude Opus 5 (claude-opus-5[1m])

### 2026-09-18 · [ad hoc] D5 fixed: a half-filled `.env` is no longer an API source
- **Change:** `app.py:58`: `active_source()` asks `get_api_client()`, the check the data routes use, instead of reading `AIRQINO_CLIENT_ID` alone. Before, with 1 to 3 of the four credentials set (any mix that included `AIRQINO_CLIENT_ID`), the badge said "API Connected", the setup banner was hidden, and the grid stayed on "Loading readings..." while every route answered 503. With a CSV uploaded too, the badge said "API Connected" over CSV readings, a second symptom §4 didn't list. Probed first on the real page (headless Chrome, a fresh app per run, every proxy variable pointed at a closed port): 4 apps (unfixed, A = the four variables read in `active_source()`, B = this fix, C = B plus a note naming the missing credentials) × 7 mixes, 28 runs. A and B matched on every mix on both surfaces (memory addresses masked); the operator picked C, whose note lands in the next commit. `tests/test_api_routes.py`: D5's strict xfail became `test_half_filled_credentials_are_not_an_api_source` (`:233`), five cases (`AIRQINO_CLIENT_ID` alone, and each 3 of 4), plus `test_half_filled_credentials_leave_csv_as_the_source` (`:246`), both above the known-defects block. `tests/test_dashboard_page.py:96-99`: T1.5 sets all four credentials; it had set only `AIRQINO_CLIENT_ID`, so it pinned D5 (learning #11). `.quality-gates.json`: `tests-passed` 129 → 135 (measured)
- **Commit/PR:** this commit
- **Session:** S20 · **Verified:** the marker removed on the unfixed code: 1 failed. The widened tests on the unfixed code: 5 failed, 1 passed (the no-`CLIENT_ID` case, which passed before). The fix: `135 passed, 2 xfailed`, exit 0, from a file with no pipe. Red-drives against the whole suite in a scratch worktree, files restored byte-identical (`shasum -c`): the fix with the original marker and T1.5, 2 failed (D5's `XPASS(strict)` and T1.5), so nothing else pinned D5; the unfixed code with the new tests, 5 failed; design A, 135 passed; a two-of-four check (`CLIENT_ID` and `CLIENT_SECRET`), 2 failed (the no-`USERNAME` and no-`PASSWORD` cases). The gate with the CSV test hidden measured 134 and the ratchet exited 2. `quality_ratchet: 2/2 pass · 0 fail · 0 unmeasured · results 8f457f1a35fd · manifest 6e62bc2f75ef`; `--precommit` exit 0 with the manifest staged. The probe's 48 vendor 502s all came from the 8 all-four runs (6 each), where the guard refused the token request; none came from a half-filled mix
- **Model:** Claude Opus 5 (claude-opus-5[1m])
...
- **Session:** S19 · **Verified:** `python3 -m pytest -q` gives `129 passed, 3 xfailed`, exit 0; `quality_ratchet: 2/2 pass · 0 fail · 0 unmeasured · results 9706974b83af · manifest 501e3c8538de`; dashboard 68/100, the same 6 flags as at Phase 0, listed from `dashboard.html` after the close-out edits; every `file:line` in the new handoff text printed against the current files; a byte count of BOM, zero-width and no-break spaces in every touched file matches `34a0a75`; `wc -l SESSION_NOTES.md` is 385; the receipt's `changelog_ref` matches this heading
- **Model:** Claude Opus 5 (claude-opus-5[1m])

### 2026-09-18 · [ad hoc] Local branch `fix/serial-reader-race` deleted
- **Change:** on the operator's direction (the landing picker), `main` was fast-forwarded to the branch (`git merge --ff-only`, `34a0a75..1af2c49`), and the fully merged local branch `fix/serial-reader-race` (tip `1af2c49`) was deleted with `git branch -d`. It was never pushed, so there was no remote branch to delete. `main` is again the only branch
- **Commit/PR:** the close-out commit (ships this entry); branch op, no commit of its own
- **Session:** S19 · **Verified:** after `git fetch`, `git merge-base --is-ancestor origin/main fix/serial-reader-race` succeeded before the merge; afterwards `git merge-base --is-ancestor` confirmed each of `052cc5a`, `b53303e`, `927dfe8`, `1af2c49` is in `main`
- **Model:** Claude Opus 5 (claude-opus-5[1m])
